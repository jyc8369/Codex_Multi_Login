...
function renderAccountCard(account) {
    ...
    <div class="${cardClass}">
      ...
      <div class="metrics-row">
        ${renderMetricCard("5-hour limit", quota?.hourlyWindowPresent ? quota.hourlyPercentage : undefined, "bar-green", renderQuotaCardMeta(quota?.hourlyWindowPresent, quota?.hourlyWindowMinutes, quota?.hourlyResetTime, quota?.hourlyRequestsLeft, quota?.hourlyRequestsLimit, planContext.hourly))}
        ${renderMetricCard("Weekly limit", quota?.weeklyWindowPresent ? quota.weeklyPercentage : undefined, "bar-yellow", renderQuotaCardMeta(quota?.weeklyWindowPresent, quota?.weeklyWindowMinutes, quota?.weeklyResetTime, quota?.weeklyRequestsLeft, quota?.weeklyRequestsLimit, planContext.weekly))}
        ${renderMetricCard("Months limit", quota?.monthlyWindowPresent ? quota.monthlyPercentage : undefined, "bar-blue", renderQuotaCardMeta(quota?.monthlyWindowPresent, quota?.monthlyWindowMinutes, quota?.monthlyResetTime, quota?.monthlyRequestsLeft, quota?.monthlyRequestsLimit, planContext.monthly))}
...
function renderMoreDetails(limits) {
    return `
    <details class="more">
...
function renderQuotaCardMeta(present, windowMinutes, resetTime, requestsLeft, requestsLimit, missingMessage) {
    if (present) {
        return renderQuotaMeta(windowMinutes, resetTime, requestsLeft, requestsLimit);
    }
    return missingMessage ?? "No data returned.";
}
function planQuotaContext(planType) {
...
function renderCreditBadge(credits) {
    if (!credits) {
